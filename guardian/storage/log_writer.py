...
_SEV_EMOJIS = {"INFO": "ℹ", "WARN": "⚠", "CRITICAL": "🚨", "EMERGENCY": "🆘"}
...
class LogWriter:
    def __init__(self, config: StorageConfig) -> None:
        ...
        if not self._human_logger.handlers:
            ...
            h.setFormatter(logging.Formatter("%(asctime)s.%(msecs)03d %(message)s", datefmt="%Y-%m-%d %H:%M:%S"))
    ...
    def log_alert(self, alert: Alert) -> None:
        ...
        self._human_logger.info("[%s] [%s] %s — %s", sev, alert.category, alert.title, metric_summary)
        record = {
            "ts": alert.timestamp,
            "severity": sev,
